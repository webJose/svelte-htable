...
sub-items.
5. The title of the elements with sub-items can be specified freely using either the value of a property or completely 
arbitrary logic using a function.
...
<script lang="ts">
    import
        ...
        CaptionOrder, // Enumerable to control the order of the data in captions
        ItemGrouping, // Enumerable to control the item grouping behavior
...
page/component file with `:global()`.  I would say this is probably the saner option so far because most likely you 
don't want to target other tables in your project by mistake.  The quickstart example uses this one.
...
| `LevelPath` | 2 | The sub-tables' captions will show the table's level to the lef and the table's path to the right. |
...
component allows the specification of a maximum path segment length to help overrunning the available space for both 
the level and the path data.  It works like this:  A path segment's value is obtained and measured agains the value 
specified in the `maxPathSegmentLength` prop.  If the length exceeds the specification, then the path segment is 
