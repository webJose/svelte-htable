...
But that's not all.  Sub-tables also get either the `sub-even` or `sub-odd` CSS classes depending on whether their 
level is odd or even.  This is helpful to achieve alternating per-table coloring.  The image shown at the beginning 
was created using these clases to apply alternating blueish backgrounds to sub-tables.
...
**summary** for obvious reasons.  Afther the summary, the sub-table is shown.
